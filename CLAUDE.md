...
- **Site routes must filter with `getPublicNotes`** (`publish !== false && draft !== true`) — `index`, `[slug]`, `tags/[tag]` all rely on it. **Tags must go through `slugifyTag`/`getTagPath`/`getTagGroups`** — raw tags in URLs break the static tag route.
- **Notes are served at the site root, `/:slug`** (not `/notes/:slug`) — the route file is `src/pages/[slug].astro`. `getPublicPathForSlug` (site), the note's canonical `pathname`, the plugin's public-URL builders (`publisher.ts`, `main.ts`), and the plugin's internal wiki-link rewriting (`markdown.ts`) must all agree on this root-level prefix. A note slug that collides with a reserved top-level path (e.g. `tags`) would shadow that route.
